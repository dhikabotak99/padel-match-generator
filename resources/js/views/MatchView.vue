<template>
  <div class="min-h-screen bg-sage-50 py-8 px-4">
    <div v-if="match" class="max-w-5xl mx-auto flex flex-col min-h-[80vh] bg-white rounded-3xl shadow-xl overflow-hidden border border-sage-100">
      <!-- Header -->
      <div class="p-6 md:p-8 bg-white border-b border-sage-100 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0 relative overflow-hidden">
        <div class="relative z-10 w-full sm:w-auto text-center sm:text-left">
            <div class="flex items-center justify-center sm:justify-start space-x-3 mb-1">
                 <h1 class="text-2xl md:text-3xl font-extrabold text-sage-900 tracking-tight">{{ match.name }}</h1>
                 <span class="bg-sage-100 text-sage-700 text-xs px-2 py-1 rounded-md font-bold uppercase">{{ match.type }}</span>
            </div>
        ...
        <!-- Decorative bg -->
        <div class="absolute right-0 top-0 h-full w-1/3 bg-gradient-to-l from-sage-50 to-transparent pointer-events-none"></div>
      </div>

      <!-- Tabs -->
      <div class="flex border-b border-sage-100 bg-sage-50/50">
        <button 
          @click="activeTab = 'games'"
          :class="{'border-b-2 border-sage-600 text-sage-800 bg-white': activeTab === 'games', 'text-sage-500 hover:text-sage-700 hover:bg-sage-50': activeTab !== 'games'}"
        ...
        <button 
          ...
          :class="{'border-b-2 border-sage-600 text-sage-800 bg-white': activeTab === 'leaderboard', 'text-sage-500 hover:text-sage-700 hover:bg-sage-50': activeTab !== 'leaderboard'}"
          class="flex-1 py-3 px-4 md:py-4 md:px-6 text-center font-bold text-base md:text-lg focus:outline-none transition-all duration-300"
        >
          Leaderboard
        </button>
          ...
          <div v-if="activeTab === 'games'" key="games">
            <div v-if="currentRound" class="space-y-6">
              <div class="flex items-center justify-between mb-2">
                  <h3 class="text-xl font-bold text-sage-800">Matchups</h3>
            ...
            <div v-else class="text-center py-20 text-sage-400">
               ...
               <button @click="nextRound" class="mt-4 text-sage-600 underline hover:text-sage-800">Start Round 1</button>
            </div>
          </div>
          <div v-else-if="activeTab === 'leaderboard'" key="leaderboard">
            <div class="overflow-x-auto bg-white rounded-2xl border border-sage-100 shadow-sm">
              <table class="min-w-full">
                <thead>
                  <tr class="bg-sage-50 text-sage-600 uppercase text-xs leading-normal">
                ...
                <tbody class="text-sage-700 text-sm">
                  <tr v-for="(player, index) in leaderboard" :key="player.id" class="border-b border-sage-50 hover:bg-sage-50/50 transition duration-150">
                    ...
                    <td class="py-4 px-4 md:px-6 text-right font-black text-lg text-sage-800">
                      {{ player.points }}
                    </td>
                     <td class="py-4 px-4 md:px-6 text-right">
                      <span class="bg-sage-100 text-sage-600 py-1 px-3 rounded-full text-xs font-bold">{{ player.gamesPlayed }}</span>
                    </td>
                  </tr>
                </tbody>
              </table>
    ...
    <div v-else class="flex flex-col justify-center items-center min-h-[80vh]">
      <div class="animate-spin rounded-full h-16 w-16 border-t-4 border-b-4 border-sage-600 mb-6"></div>
       <p class="text-sage-500 font-medium text-lg">Loading match...</p>
    </div>
  </div>
</template>
