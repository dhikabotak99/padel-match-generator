<template>
  <div class="min-h-screen bg-sage-50 py-12 px-4">
    <div v-if="match" class="max-w-4xl mx-auto bg-white shadow-xl rounded-3xl overflow-hidden border border-sage-100 relative">
      <!-- Decorative Header -->
      <div class="h-24 md:h-32 bg-sage-600 relative overflow-hidden">
        <div class="absolute inset-0 bg-sage-700 opacity-20 transform -skew-y-3"></div>
      </div>
      ...
      <div class="px-6 md:px-8 pb-8 relative -mt-10 md:-mt-12">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 space-y-4 md:space-y-0">
            <div>
                <div class="bg-white p-2 rounded-xl shadow-md inline-block mb-3">
                    <span class="bg-sage-100 text-sage-800 px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wide">{{ match.type }}</span>
                </div>
                <h2 class="text-2xl md:text-4xl font-extrabold text-sage-900 leading-tight">{{ match.name || 'Untitled Match' }}</h2>
                <div class="flex items-center space-x-2 mt-2 text-sage-600 text-sm font-medium">
                    <span>{{ match.scoring_type === '21' ? 'Points (21)' : 'Tennis Rules' }}</span>
                    <span>•</span>
                    <span :class="{'text-yellow-600': match.status === 'pending', 'text-green-600': match.status === 'active'}" class="capitalize">{{ match.status }}</span>
                </div>
            </div>
        ...
        <div>
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-xl font-bold text-sage-800">Players</h3>
                <span class="bg-sage-100 text-sage-600 px-3 py-1 rounded-full text-sm font-bold">{{ match.players.length }} registered</span>
            </div>
